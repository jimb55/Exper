...
/**
 * RSA 加密
 * 来源 ： https://blog.csdn.net/mengke1124/article/details/52326142
 *
 * Class rsa
 */
...
class rsa {
     ...
     */
    static public function create_key() {
        $res = openssl_pkey_new();
        if($res == false) return false;
        openssl_pkey_export($res, $private_key);
        $public_key = openssl_pkey_get_details($res);
        return array($public_key["key"],$private_key);
    }
    /**
     * 用私密钥加密
     */
...
//创建公私钥
if(!file_exists('pub') || !file_exists('pri')){
    list($public_key,$private_key) = rsa::create_key();
    shell_exec("echo '".$public_key."' > pub");
    shell_exec("echo '".$private_key."' > pri");
}
